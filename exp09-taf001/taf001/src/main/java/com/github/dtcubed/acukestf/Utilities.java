...
public class Utilities {
        ...
        throws Throwable
    {
        ...
        logMsg = String.format("RUN Feature File: [%s]",  runFeatureFile);
        logger.info(logMsg);
        ...
        try {
            ...
            for (File feature_file : file_list) {
                ...
                logger.debug("Canonical Path: " + feature_file.getCanonicalPath());
                logger.debug("Relative Path : " + feature_file.getPath());
                ...
                // 20170710 - dworden - using Unix convention 
                cummulativeFFcontents += " \n \n";

            }

            logger.trace("START CUMMULATIVE FF CONTENTS: [");
            logger.trace(cummulativeFFcontents);
            logger.trace("] END CUMMULATIVE FF CONTENTS");
            ...
            logger.debug("Suite Production Flag: [" + Boolean.toString(suiteProduction) + "]");
            logger.debug("Suite Stop On Failure: [" + Boolean.toString(suiteStopOnFailure) + "]");
            logger.debug("Suite Version        : [" + suiteVersion + "]");
            ...
            // bw.write("\r\n");
            // bw.write("\r\n");
            // 20170711 - dworden - using Unix convention 
            bw.write("\n");
            bw.write("\n");
            ...
            for(int testCaseIndex = 0; testCaseIndex < testCase.length(); testCaseIndex++) {
                ...
                for(int repetitionCount = 0; repetitionCount < testCaseCount; repetitionCount++) {
                    ...
                    //bw.write("\r\n");
                    // Write an additional UNIX "newline" after each Scenario.
                    bw.write("\n");
                }
    ...
    public static boolean execute_run_feature_file(String runDirectory) throws Throwable
    {
        ...
        logger.debug("Executing Feature File : [" + runFeatureFile + "]");
        logger.debug("Cucumber Plugin Value 1: [" + cucumberPlugInValueOne + "]");
        logger.debug("Cucumber Plugin Value 2: [" + cucumberPlugInValueTwo + "]");
    ...
    public static boolean execute_test_cases(String testSuiteFile, String featureFileBaseDir) throws Throwable
    {
        ...
        logger.info("Processing Test Suite: [" + testSuiteFile + "]");
        ...
        try {
            ...
            logger.trace("START TEST SUITE FILE CONTENTS: [");
            logger.trace(fileContents);
            logger.trace("] END TEST SUITE FILE CONTENTS");
            ...
            for(int testCaseIndex = 0; testCaseIndex < testCase.length(); testCaseIndex++) {
                ...
                logger.debug("Test Case Count: [" + Integer.toString(testCaseCount) + "]");

                testCaseReturnCode = CucumberCliApiWrapper.doit(testCaseNameWithTag, featureFileBaseDir);

                logger.debug("TEST CASE EXIT STATUS: [" + Byte.toString(testCaseReturnCode) + "]");
